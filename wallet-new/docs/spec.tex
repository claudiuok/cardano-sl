...
for some reason they do not get included. This functionality does not need to be
part of the wallet proper; instead we model it abstractly as shown in
Figure~\ref{fig:transaction_submission_layer}. Just like the wallet expects to
be notified of events such as `new block arrived', `user submitted a new
transaction', the submission layer expects to be notified when the set of
pending transactions grows or shrinks.
...
$\mathit{pending}$ set in all checkpoints, however, a rollback won't reintroduce
it into $\mathit{pending}$; if the user wants to explicitly tell the wallet to
try this transaction again they will need to call $\mathsf{newPending}$.
Effectively, $\mathsf{cancel}$ becomes a secondary way in which a transaction
may go from $\texttt{Applying}$ to $\texttt{WontApply}$ (arrow
$\mathsf{applyBlock}^*$), and $\mathsf{newPending}$ a secondary way to get back
from $\texttt{WontApply}$ to $\texttt{Applying}$ (arrow $\mathsf{rollback}^*$).

\subsection{Implementation}

In this section we give a simple implementation of the submission layer, shown
in Figure~\ref{fig:submission_layer_impl}. The primary goal is to show that the
...
of the wallet and a schedule of which transactions to (re)submit next. The
schedule is modelled as a simple list of time slots, recording for each slot the
transactions that should be submitted along with a submission count for each
transaction.
%
\begin{itemize}
...
\item At the start of each time slot, the submission layer relies on the
``resubmission parameter'' $\varrho$ to resubmit the set of transactions
that are due, possibly dropping some transactions that have reached a
...
pending).
\item Resubmit the remainder to the network, and then reschedule them for
the next attempt later. The submission count can be used to implement an
exponential back-off if desired.
\end{itemize}
...
of a linear scan to reschedule transactions is unacceptable it can of course
easily use different datatype (such as a fingertree).
...
\end{align*}
%
\caption{\label{fig:submission_layer_impl}Submission layer implementation (sketch)}
\end{figure}
...
\section{Transaction resubmission and cancellation (OLD)}
